...
function isPalindrome(str) {
  const res = str.toLowerCase().replace(/\W/g, '');
...
function findLongestWord(sentence) {
  const maxLength = Math.max(...sentence.split(' ').map((el) => el.length));
  return sentence.split(' ').filter((el) => el.length === maxLength)[0];
}
...
function invertCase(str) {
  return str
    .split('')
    .map((el) => (el.charCodeAt(0) < 97 ? el.toLowerCase() : el.toUpperCase()))
    .join('');
}
...
function extractNameFromTemplate(value) {
  ...
  let surname = temp[temp.length - 1];
...
function encodeToRot13(str) {
  let res = str.split('').map((el) => el.charCodeAt(0));
  for (let i = 0; i < res.length; i += 1) {
    const temp = res[i];
    if (temp > 64 && temp < 78) res[i] += 13;
    if (temp > 77 && temp < 91) res[i] -= 13;
    if (temp > 96 && temp < 110) {
      res[i] += 13;
    }
    if (temp > 109 && temp < 123) res[i] -= 13;
  }
  res = res.map((el) => String.fromCharCode(el));
  return res.join('');
}
